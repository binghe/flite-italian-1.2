...
static cst_utterance *irstifd_ita_pc_postlex(cst_utterance *u);

//NO necessary for MBROLA
//extern cst_diphone_db irstifd_ita_pc_db;

cst_voice *irstifd_ita_pc_diphone = NULL;

cst_voice *register_irstifd_ita_pc(const char *voxdir)
{
    cst_voice *v = new_voice();

    /* Sets up language specific parameters in the irstifd_ita_pc. */
    italian_init(v);

    feat_set_string(v->features,"name","irstifd_ita_pc");

  
    
    //painte
    feat_set_float(v->features,"int_f0_target_mean",157.222);// hz2mel(104.792) //110.0);
    feat_set_int(v->features,"pitch_male2female_conversion",0);
    ...
    //feat_set(v->features,"postlex_func",uttfunc_val(&irstifd_ita_pc_postlex));
    ...
    if (!mbrola_dbs)
      mbrola_db="it3/it3"; //se non specifichi nulla dalla dir corrente.
    else
      mbrola_db=strcat(mbrola_dbs,"/it3/it3");
    ...
    feat_set_int(v->features,"sample_rate",irstifd_ita_pc_db.sts->sample_rate);
    ...
    */
    irstifd_ita_pc_diphone = v;
    return irstifd_ita_pc_diphone;
}

void unregister_irstifd_ita_pc(cst_voice *vox)
{
    if (vox != irstifd_ita_pc_diphone)
	return;
    delete_voice(vox);
    irstifd_ita_pc_diphone = NULL;
}
